...
{State each hard "never do this" rule this skill's output must follow, as a standalone rule rather than buried in Workflow, plus a default-for-ambiguity rule: what to do when a choice doesn't clearly map to any rule in this skill — ask, or fall back to the simplest compliant option and flag the choice.}
