...
- **Defined-term capitalization** — any term defined in the Definitions section is written in ALL CAPS every time it appears in the Requirement text (e.g. if "PAYLOAD" is defined, the requirement uses "PAYLOAD," not "payload") — signals the word carries the document's specific defined meaning rather than its everyday sense.
- **Template keyword emphasis** — italicize the Gherkin-style structural keywords — `Given`, `When`, `Then` — in the Requirement text so they stay visually distinct from the requirement's actual content, including ALL-CAPS defined terms. `SHALL` is itself a defined term (see Definitions), so it's already distinguished by the Defined-term capitalization rule, not emphasis. In docx output this is true italic formatting. In TSV output, wrap the keyword in underscores (e.g. `_Given_`) since plain text can't carry italics — the markup stays in the cell even if a given viewer doesn't render it as emphasis.
...
- **Notes/Comments** — short, to-the-point free text: background info not captured by Rationale (e.g. source of the request, related constraints), dependencies, and — for retired requirements — the version made obsolete (see obsolete handling below).
...
**Obsolete requirements are never deleted from the table.** A requirement is retired by replacing its Requirement cell with `OBSOLETE` (the full cell content, not a prefix on the original text) and clearing the Name, Rationale, Verification Method, Importance, and Tags cells for that row. Notes/Comments records only the version the requirement was obsoleted in — the reason for the change belongs in that version's Revision History entry (Reason for Change), not in Notes/Comments. The row keeps three filled cells: ID, Requirement (`OBSOLETE`), and Notes/Comments (version). That ID is retired permanently — if the same need comes back later, give it a new ID and reference the old one, rather than reusing or un-flagging the retired one.
...
- **Cross-reference** — a Requirement ID is cited only in the Revision History table's Requirement ID column and in Notes/Comments — never anywhere else in the document. A citation elsewhere creates silent drift: if the cited requirement is later obsoleted, renumbered, or rewritten, the citing text goes stale with nothing to flag it. Flag any Requirement ID found outside those two locations and recommend restating the cited rule in plain terms instead.
- **Defined-term capitalization** — a term defined in Definitions is used in Requirement text without ALL CAPS, or an ALL CAPS word appears in Requirement text that isn't a defined term or a structural keyword (`SHALL`) — flag either direction (see "Writing each column" — Defined-term capitalization).
